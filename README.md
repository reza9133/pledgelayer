...
- **Address (default):** `0x125d81a2D2cd095e9De960895e81b36291E6f6F1` — overridable via `NEXT_PUBLIC_CONTRACT_ADDRESS`
...
1. **`get_campaign` / `get_milestone` field shapes aren't verified against a live chain.** `lib/genlayerClient.ts` checks both snake_case and camelCase keys defensively, but before you trust it in production, run a real `readContract` call against Bradbury and confirm the wire shape matches `normalizeCampaign`/`normalizeMilestone`.
